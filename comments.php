...
<div id="comments" class="comments-area _comments">
	...
	if ( have_comments() ) : ?>
		<h2 class="comments-title _comments__title">
			<?php
				printf( // WPCS: XSS OK.
					esc_html( _nx( '%1$s thought on &ldquo;%2$s&rdquo;', '%1$s thoughts on &ldquo;%2$s&rdquo;', get_comments_number(), 'comments title', '_bem' ) ),
	...
	'url' =>
	  '<p class="comment-form-url _comments__field"><label for="url" class="_comments__field-label">' . __( 'Website', '_bem' ) . '</label><input class="_comments__field-input _comments__field-input--website" id="url" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) .
	  '" size="30" /></p>',
	);
	...
	comment_form(array(
		...
		'</label><textarea class="_comments__field-input _comments__field-input--comment" id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p>',
